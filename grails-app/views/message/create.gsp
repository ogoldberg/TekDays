...
<body>
  <div class="nav">
    ...
    <span class="menuButton"><g:link class="list" action="list" id="${messageInstance?.event?.id}">
        Message List
      </g:link>
    </span>
  </div>
      ...
      <div class="dialog">
        <table>
         ...
         <textarea class="messageField" rows="5" cols="60" name="content">
          ...
          <g:select name="author.id" from="${TekUser.list()}" optionKey="id" value="${messageInstance?.author?.id}"  />
          </td>
          </tr>
